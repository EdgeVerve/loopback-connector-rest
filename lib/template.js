  ...
  traverse(this.template).reduce(function(schema, item) {
    ...
    function parse(item) {
      ...
      while (match = templates.exec(item)) {
        ...
        // When an object's path is referenced in the template, for example
        // callContext.$.ctx.tenantId , this would add only "callContext" to 
        // the schema
        var jsonpathIndex = name.indexOf('.$');
        if (jsonpathIndex > 0) {
          name = name.substring(0, jsonpathIndex);
        }
    ...
    return traverse(obj).map(function(item) {
      ...
      function build(item) {
        ...
        while (match = templates.exec(item)) {
          ...
          // When val is an object for example callContext, and name refers
          // to a path for example callContext.$.ctx.tenantId , this code will
          // replace the actual value from the object to the template

          var jsonpathIndex = name.indexOf('.$');
          var path = null;
          if (jsonpathIndex > 0) {
            path = name.substring(jsonpathIndex+1);
            name = name.substring(0, jsonpathIndex);
          }
