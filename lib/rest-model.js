...
function RestResource(pluralModelName, baseUrl, requestFunc) {
  ...
  this.pluralModelName = this.pluralModelName;
  ...
  if (cb) {
    callback = function(err, response, body) {
      ...
      if (!err && response && response.statusCode >= 400) {
        ...
        var result = body;
        if (template && template.template.responsePath) {
            result = _.get(body, template.responsePath);
        }
...
RestResource.prototype.create = function(obj, options, cb) {
  ...
  var template = this.connector.restTemplates['create'];
  var req = this.build(template, {
    body : obj,
    callContext:options
  });
  
  req.method = req.method || 'POST';
  req.uri = req.uri ||  this._url;
  req.json = req.json || true;
  req.body = req.body || obj;
  
  this._request(req, options, wrap(template, cb));
 ...
 * @param {function} [cb] The callback function
 */
RestResource.prototype.updateAll = function(query, obj, options, cb) {
  ...
  var template = this.connector.restTemplates['updateAll'];
  var req = this.build(template, {
    qs : query,
    body : obj,
    callContext:options  
  ...
  req.method = req.method || 'POST';
  req.uri = req.uri ||  this._url + '/update';
  req.json = req.json || true;
  req.body = req.body || obj;
  req.qs = req.qs || query;
  this._request(req, options, wrap(template, cb));
...
RestResource.prototype.update = function(id, obj, options, cb) {
  ...
  var template = this.connector.restTemplates['update'];
  var req = this.build(template, {
    callContext:options,
    id : id,
    body : obj
...
RestResource.prototype.delete = function(id, options, cb) {
  ...
  var template = this.connector.restTemplates['deleteById'];
  var req = this.build(template, {
    id : id,
    callContext:options
  });
  
  req.method = req.method || 'DELETE';
  req.uri = req.uri ||  this._url + '/' + id;
  req.json = req.json || true;
  this._request(req, options, wrap(template, cb));
...
RestResource.prototype.deleteAll = function(options, cb) {
  ...
  var template = this.connector.restTemplates['deleteAll'];
  var req = this.build(template, {
    callContext:options  
  });
...
RestResource.prototype.find = function(id, options, cb) {
  ...
  var template = this.connector.restTemplates['findById'];
  var req = this.build(template, {
    id : id,
    callContext:options
  });
...
RestResource.prototype.all = RestResource.prototype.query = function(q, options, cb) {
  ...
  var template = this.connector.restTemplates['query'];
  var req = this.build(template, {
    callContext:options,
    qs : q
  });
...
RestResource.prototype.build = function(template, obj) {
    var req = {
        url : this._url,
        default : true
    };
    obj.modelName = this.modelName;
    obj.modelUrl = this.pluralModelName;
    if (template) {
        return template.build(obj);
    }
    else {
        return req;
    }
