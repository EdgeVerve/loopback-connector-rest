...
RestConnector.prototype.update = function update(model, query, data, callback) {
  if (query.where) {
    this.getResource(model).update(data.id, data, this.responseHandler(model, callback));
  } else {
    this.getResource(model).updateAll(query, data, this.responseHandler(model, callback));
  }
};
