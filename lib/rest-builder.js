...
RequestBuilder.prototype.operation = function(parameterNames) {
  ...
  return function() {
    ...
    var model = this; 
    return self.invoke(parameters, function(err, data, response, dataSourceSettings) {
        var models ;
        if (dataSourceSettings && dataSourceSettings.crud) {
          if (Array.isArray(data)) {
            models = [];
            data.forEach(function(item) {
                models.push(new model(item, {applySetters: false, persisted: true}));
            })  
          } else {
              models = new model(data, {applySetters: false, persisted: true});
          }
          cb(err, models, response);
        }
        else {
