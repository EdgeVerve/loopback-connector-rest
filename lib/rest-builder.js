...
/**
 * Map the request builder to a connector function
 ...
 */
RequestBuilder.prototype.connectorOperation = function(paramNames, operation) {
  var params = [];
  Array.prototype.slice.call(paramNames).forEach(function(a) {
    if ('string' === typeof a) {
      params.push(a);
  ...
  // here self will be request builder
  var self = this;
  return function() {
    var parameters = {};
    var cb = null;
    ...
    return self.invoke(parameters, optionsArgument, function(err, data, response, dataSourceSettings) {
        var models ;
 ...
 * @returns {function} A function to invoke the REST operation
 */
RequestBuilder.prototype.operation = function(parameterNames, isPrototypeMethod) {
  var params = [];
  ...
  return function() {
    ...
    var model = isPrototypeMethod ? this.constructor : this;
    if (!parameters['model']) {
    ...
    return self.invoke(parameters, optionsArgument, function(err, data, response, dataSourceSettings) {
        var models ;
        if (dataSourceSettings && dataSourceSettings.crud) {
          if (Array.isArray(data)) {
		        	data.map(x=>new model(x, {applySetters: false, persisted: true}));
          } else {
            data = new model(data, {applySetters: false, persisted: true});
          }
...
RequestBuilder.prototype.invoke = function(parameters, options, cb) {
    ...
    callback = function(err, response, body) {
      if (err) {
        ...
        if (jsonPath && self.template.template.responsePath) {
          ...
          // });
          result = _.get(body, self.template.template.responsePath);
        }
        if (debug.enabled) {
          debug('Response: %j', result);
        }
        return cb(err, result, response, self.connector.dataSource.settings);
      }
    };
  }
  ...
  var r = this._request(req, options, callback);
  return cb.promise;
};
 ...
 * @param {options} CallContext options
 * @param {function} cb The callback function
 */
